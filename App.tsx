  ...
  return (
    <div className={`${isDarkTheme ? 'dark liquid-bg text-white' : 'bg-[#F2F2EC] text-primary'} h-screen w-screen overflow-hidden flex justify-center transition-colors duration-500 font-sans`}>
      ...
      <NotificationContext.Provider value={{ openNotifications }}>
        {/* Main App Container */}
        <div className={`relative w-full h-full max-w-md flex flex-col shadow-2xl overflow-hidden md:rounded-[24px] md:my-4 md:h-[calc(100vh-2rem)] md:border md:border-black/5 ${isDarkTheme ? 'glass-panel ring-1 ring-white/5' : 'bg-[#F2F2EC]'}`}>
            ...
            {isMemberRoute && !isAdminRoute && user && (
              <div className="absolute bottom-6 left-0 right-0 flex justify-center z-30 px-4">
                 <nav className="w-full glass-card rounded-2xl p-1.5 flex items-stretch justify-between shadow-glass backdrop-blur-2xl bg-[#0f120a]/80 border border-white/10 h-16">
