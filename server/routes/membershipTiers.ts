...
router.put('/api/membership-tiers/:id', isAdmin, async (req, res) => {
  try {
    ...
    const {
      ...
      sim_hours_limit, guest_passes_per_month, booking_window_days,
    ...
    const result = await pool.query(`
      UPDATE membership_tiers SET
        ...
        sim_hours_limit = COALESCE($11, sim_hours_limit),
...
router.post('/api/membership-tiers', isAdmin, async (req, res) => {
  try {
    ...
    `, [
      ...
      sim_hours_limit || 0, guest_passes_per_month || 0, booking_window_days || 7,
