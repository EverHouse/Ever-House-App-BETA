  ...
  return (
    <div className="relative min-h-screen pb-24">
      ...
      <div className="px-6 space-y-8">
        {itemsByCategory.map(cat => (
          <div key={cat.category} className={activeCategory === cat.category ? 'block' : 'hidden'}>
            ...
            {isOrderPlaced ? (
              ...
              <>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setIsCartOpen(false)} className="p-2 rounded-lg glass-button hover:bg-white/10 text-white active:scale-90">
                    <span className="material-symbols-outlined">close</span>
                  </button>
                </div>
                    ...
                    <div key={item.id} className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5 animate-pop-in" style={{animationDelay: `${idx * 0.05}s`}}>
                      <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3 bg-white/5 rounded-lg px-2 py-1">
                          <button onClick={() => removeFromCart(item.id)} className="text-white/60 hover:text-white active:scale-90">
                            <span className="material-symbols-outlined text-sm">remove</span>
                          </button>
                          <span className="text-sm font-bold w-4 text-center text-white">{item.qty}</span>
                          <button onClick={() => addToCart(item)} className="text-white/60 hover:text-white active:scale-90">
