      ...
      <section className="mb-8 border-b border-white/10 -mx-6 px-6">
        <div className="flex gap-6 overflow-x-auto pb-0 scrollbar-hide">
          <button 
            ...
            className={`pb-3 border-b-[3px] ${activeTab === 'classes' ? 'border-white text-white font-bold' : 'border-transparent text-white/60 font-medium'} text-sm whitespace-nowrap transition-colors`}
          >
            Upcoming Classes
          </button>
          <button 
            onClick={() => setActiveTab('medspa')}
            className={`pb-3 border-b-[3px] ${activeTab === 'medspa' ? 'border-white text-white font-bold' : 'border-transparent text-white/60 font-medium'} text-sm whitespace-nowrap transition-colors`}
          >
            MedSpa
          </button>
        </div>
...
const DateButton: React.FC<{day: string; date: string; active?: boolean; onClick?: () => void}> = ({ day, date, active, onClick }) => (
  <button onClick={onClick} className={`flex-shrink-0 flex flex-col items-center justify-center w-16 h-20 rounded-2xl transition-transform active:scale-95 ${active ? 'bg-white text-brand-green shadow-glow' : 'glass-button text-white'}`}>
