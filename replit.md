...
- **Auto-Mapping Booking Logic**: Authorized members (Core, VIP, Premium, Corporate tiers or Founding Member tag) can book simulators directly without payment. Unauthorized members (Social tier without Founding tag) receive a 402 response directing them to contact the front desk. The `MEMBER_BOOKING_PRODUCT_ID` constant in `server/core/trackman.ts` stores the Trackman product ID for member bookings.
