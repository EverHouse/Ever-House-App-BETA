...
- **Booking System**: Supports "Request & Hold" (staff approval required for some tiers), simulator and conference room booking requests with conflict detection, and staff-initiated manual bookings. Includes calendar management for staff to reschedule or cancel bookings. **Member Reschedule Workflow**: Members can reschedule existing approved bookings via the "Reschedule" button on their dashboard. This creates a new booking request with `reschedule_booking_id` linking to the original. When staff approves the reschedule, the original booking is automatically cancelled and its Google Calendar event deleted. Reschedules bypass daily booking limits (member is moving, not adding). 30-minute minimum advance time required for rescheduling. Duplicate reschedule requests are prevented. Conference room bookings on the same day as a bay booking filter out overlapping time slots.
