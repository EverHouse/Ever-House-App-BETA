...
- **Error Handling**: Generic error messages to prevent info leaks. Structured JSON logging with request IDs for debugging - every API response includes a `requestId` that can be matched to server logs.
...
- **Staff Manual Booking**: Staff can create bookings directly on behalf of members via the Sims admin panel. Tracks booking source (Trackman, YGB, Mindbody, Texted Concierge, Called, Other), guest count, and which staff member created the booking. Automatically increments member's lifetime visits and syncs to Google Calendar.
- **Booking Calendar Management**: Clickable calendar cells open a details modal showing member name, email, time, duration, bay, and booking metadata. Staff can reschedule (pre-fills manual booking form with member email) or cancel directly. Cancellations automatically notify the member. "Upcoming Bookings" section at top of Queue tab shows next 10 confirmed bookings for quick access.
